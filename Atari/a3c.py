...
# import visdom
# vis = visdom.Visdom()
# vis.delete_env('Atari_Plots') #If you want to clear all the old plots for this python Experiments.Resets the Environment
# vis = visdom.Visdom(env='Atari_Plots')
os.environ['OMP_NUM_THREADS'] = '1'

def save_dic(name,data):
    with open('./New_Scores/r'+str(name), "wb") as output_file:
...
def load_dic(name):
    with open(name, "rb") as input_file:
        ...
        return e
cdata= {k: torch.Tensor([0]).share_memory_() for k in range(20)}
def get_args():
    ...
    parser.add_argument('--ipname', default='none', type=str, help='hidden size of GRU')
    parser.add_argument('--input', default='Orginal', type=str, help='hidden size of GRU')
    return parser.parse_args()

discount = lambda x, gamma: lfilter([1],[1,-gamma],x[::-1])[::-1] # discounted rewards one liner
prepro = lambda img: imresize(img[35:195].mean(2), (80,80)).astype(np.float32).reshape(1,80,80)/255.
collect_data=[[] for _ in range(20)]
...
def train(shared_model, shared_optimizer, rank, args, info,collect_data_p,name):
    global cdata
    ...
    while info['frames'][0] <= 8e7 or args.test: # openai baselines uses 40M frames...we'll use 80M
        ...
        for step in range(args.rnn_steps):  
            ...
            #each time we come here we have finished an episode
            if done: # update shared data
                info['episodes'] += 1
                interp = 1 if info['episodes'][0] == 1 else 1 - args.horizon
                #print("no of episodes is ",rank,info['episodes'].item())
                # print("EPR: ", epr)
                info['run_epr'].mul_(1-interp).add_(interp * epr)
                #collect_data_p.append(epr)
                collect_data[rank].append(epr)
                save_dic(name+'_'+str(rank),collect_data[rank])
                #L.append(epr)
                # print("INFO_RUN_EPR: ", info['run_epr'].mul_(1-interp).add_(interp * epr))
...
if __name__ == "__main__":
    ...
    args.save_dir = './CAE_Recollection'
    ...
    # info['frames'] += shared_model.try_load("breakout_0.997068769558176.pt")# * 1e6
    folders=['Trained_nets','Latest_Cae','New_Games']
    if args.input=='Orginal':
        selected_folder=folders[0]
    else:
        selected_folder=folders[2]

    print("Using Folder {} and file {} inside it".format(str(selected_folder),str(args.ipname)))
    orginal_nets=['spaceinvaders-v4.40.tar','pong-v4.40.tar','breakout-v4.40.tar','kangaroo.40.tar','stargunner.40.tar']
    shared_model.load_state_dict(torch.load("./"+str(selected_folder)+"/"+str(args.ipname)))
    #shared_model.load_state_dict(torch.load("./CAE_Recollection/spaceinvad_100_0.9930222248094316.pt"))# * 1e6
    info['frames'] = torch.Tensor([0])
    if int(info['frames'].item()) == 0: printlog(args,'', end='', mode='w') # clear log file
    
    processes = []
    for rank in range(args.processes):
        p = mp.Process(target=train, args=(shared_model, shared_optimizer, rank, args, info,collect_data[rank],str(selected_folder)+"_"+str(args.ipname[0:14])))
